...
public enum ST_AlignType {
    ...
    private func layoutAttributes(isInner: Bool, isVertical: Bool) -> ST_LayoutAttributes {
        let attributes = ST_LayoutAttributes()
        ...
        switch self {
            ...
            // 仅内部参照需要
            case .Center:
                return ST_LayoutAttributes(horizontal: .CenterX, referHorizontal: .CenterX, vertical: .CenterY, referVertical: .CenterY)
        }
    }
}
...
extension UIView {
    ...
    :returns: 约束数组
    */
    public func ST_Fill(referView: UIView, insets: UIEdgeInsets = UIEdgeInsetsZero) -> [NSLayoutConstraint] {
    ...
    :returns: 约束数组
    */
    public func ST_AlignInner(type type: ST_AlignType, referView: UIView, size: CGSize?, offset: CGPoint = CGPointZero) -> [NSLayoutConstraint]  {
        
        return ST_AlignLayout(referView, attributes: type.layoutAttributes(true, isVertical: true), size: size, offset: offset)
    }
    ...
    :returns: 约束数组
    */
    public func ST_AlignVertical(type type: ST_AlignType, referView: UIView, size: CGSize?, offset: CGPoint = CGPointZero) -> [NSLayoutConstraint] {
        
        return ST_AlignLayout(referView, attributes: type.layoutAttributes(false, isVertical: true), size: size, offset: offset)
    }
    ...
    :returns: 约束数组
    */
    public func ST_AlignHorizontal(type type: ST_AlignType, referView: UIView, size: CGSize?, offset: CGPoint = CGPointZero) -> [NSLayoutConstraint] {
        
        return ST_AlignLayout(referView, attributes: type.layoutAttributes(false, isVertical: false), size: size, offset: offset)
    }
    ...
    :returns: 约束数组
    */
    public func ST_HorizontalTile(views: [UIView], insets: UIEdgeInsets) -> [NSLayoutConstraint] {
        ...
        firstView.ST_AlignInner(type: ST_AlignType.TopLeft, referView: self, size: nil, offset: CGPoint(x: insets.left, y: insets.top))
        ...
        for i in 1..<views.count {
            let subView = views[i]
            cons += subView.ST_sizeConstraints(firstView)
            subView.ST_AlignHorizontal(type: ST_AlignType.TopRight, referView: preView, size: nil, offset: CGPoint(x: insets.right, y: 0))
            preView = subView
        }
    ...
    :returns: 约束数组
    */
    public func ST_VerticalTile(views: [UIView], insets: UIEdgeInsets) -> [NSLayoutConstraint] {
        ...
        for i in 1..<views.count {
            let subView = views[i]
            cons += subView.ST_sizeConstraints(firstView)
            subView.ST_AlignVertical(type: ST_AlignType.BottomLeft, referView: preView, size: nil, offset: CGPoint(x: 0, y: insets.bottom))
            preView = subView
        }
    ...
    :returns: 对应的约束
    */
    public func ST_Constraint(constraintsList: [NSLayoutConstraint], attribute: NSLayoutAttribute) -> NSLayoutConstraint? {
    ...
    :returns: 约束数组
    */
    private func ST_AlignLayout(referView: UIView, attributes: ST_LayoutAttributes, size: CGSize?, offset: CGPoint) -> [NSLayoutConstraint] {
        
        translatesAutoresizingMaskIntoConstraints = false
        
        var cons = [NSLayoutConstraint]()
        
        cons += ST_positionConstraints(referView, attributes: attributes, offset: offset)
        
        if size != nil {
            cons += ST_sizeConstraints(size!)
        }
    ...
    :returns: 约束数组
    */
    private func ST_sizeConstraints(size: CGSize) -> [NSLayoutConstraint] {
    ...
    :returns: 约束数组
    */
    private func ST_sizeConstraints(referView: UIView) -> [NSLayoutConstraint] {
    ...
    :returns: 约束数组
    */
    private func ST_positionConstraints(referView: UIView, attributes: ST_LayoutAttributes, offset: CGPoint) -> [NSLayoutConstraint] {
...
///  布局属性
private final class ST_LayoutAttributes {
